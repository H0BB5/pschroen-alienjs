import { Color, Easing, Interface, Stage, ticker, clearTween, delayedCall, tween, clamp } from '@hobbs/alien-core';
...
export class PanelGraph extends Interface {
    ...
    noHover: boolean;
    noGradient: boolean;
    callback?: (value: any, target: PanelGraph) => any;
    ...
    lineColors: { graph: string; bottom: string; handle: string };
    colorRange: Color[];
    colorStep: number;
    color: Color;
    alpha: number;
    ...
    prev?: number;
    fps?: number;
    refreshRate120?: number;
    refreshRate240?: number;

    handleProps: { alpha: number };
    timeout?: any;
    ...
    constructor({
        ...
        range = 1,
        suffix = '',
        format = (value: string) => `${value}${suffix}`,
        value,
        ghost,
        noText = false,
        noHover = false,
    ...
    initCanvas() {
        ...
        this.context = (this.canvas.element as HTMLCanvasElement).getContext('2d')!;
        this.add(this.canvas);
    }

    createGradient(x0: number, y0: number, x1: number, y1: number, alpha: number = 1): CanvasGradient {
        this.alpha = alpha;
    ...
    toRGBA(color: Color, alpha: number): string {
    ...
    addListeners() {
        if (!this.noHover) {
            ...
            window.addEventListener('pointerdown', this.onPointerDown as EventListener);
    ...
    removeListeners() {
        if (!this.noHover) {
            ...
            window.removeEventListener('pointerdown', this.onPointerDown as EventListener);
    ...
    getRangeHeight(range: number): number {
    ...
    resize() {
        ...
        (this.canvas.element as HTMLCanvasElement).width = Math.round(this.width * dpr);
        (this.canvas.element as HTMLCanvasElement).height = Math.round(this.height * dpr);
        (this.canvas.element as HTMLCanvasElement).style.width = `${this.width}px`;
        (this.canvas.element as HTMLCanvasElement).style.height = `${this.height}px`;
    ...
    updateGraph(value?: number | number[]) {
        if (value !== undefined) {
            if (Array.isArray(value)) {
                this.setArray(value);
            } else {
    ...
    drawGraph() {
        ...
        this.context.clearRect(0, 0, (this.canvas.element as HTMLCanvasElement).width, (this.canvas.element as HTMLCanvasElement).height);
        ...
        if (!this.noHover) {
            ...
            const y = h - value * this.rangeHeight - 1;
    ...
    drawPath(h: number, array: number[], ghost?: boolean) {
        if (ghost) {
