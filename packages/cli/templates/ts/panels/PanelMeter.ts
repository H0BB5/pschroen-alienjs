import { Color, Easing, Interface, Stage, ticker, defer } from '@hobbs/alien-core';
...
export class PanelMeter extends Interface {
    ...
    noText: boolean;
    noGradient: boolean;
    callback?: (value: any, target: PanelMeter) => any;
    ...
    lineColors: { graph: string; bottom: string };
    colorRange: Color[];
    colorStep: number;
    color: Color;
    alpha: number;
    ...
    async init() {
        ...
        if (!this.noText) {
            ...
            this.info.text(this.format(Number(0).toFixed(this.precision)));
            (this.info as any).width = 0;
            this.add(this.info);

            await defer();

            (this.info as any).width = this.info.element!.getBoundingClientRect().width;
    ...
    getRangeWidth(range: number): number {
    ...
    setValue(value?: number) {
        ...
        if (this.info) {
            ...
            if (x + (this.info as any).width > this.width) {
                x = this.width - (this.info as any).width;
            }
    ...
    drawPath(y: number, value: number, ghost?: boolean) {
        if (ghost) {
