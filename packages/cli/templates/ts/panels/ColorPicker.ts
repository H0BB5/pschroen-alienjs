import { Color, Interface, Stage, Vector2, PI, PI60, PI90, Third, TwoPI, brightness, clamp, radToDeg } from '@hobbs/alien-core';
...
export class ColorPicker extends Interface {
    ...
    constructor({
        ...
        this.name = name;
        this.value = new Color(value as any);
        ...
        if (!this.noSwatch || !this.noText) {
            ...
            if (!this.noSwatch) {
                ...
                this.swatch.css({
                    ...
                    border: 'var(--ui-color-picker-swatch-border)',
                    cursor: 'pointer'
                });
    ...
    initColorRing() {
        ...
        this.colorRing = new Interface(null, 'svg') as any;
        this.colorRing.hide();
        this.colorRing.attr({
        ...
        let colors: { offset: string; stopColor: string; stopOpacity: number }[];
        let gradient: Interface;
        ...
        for (let i = 0; i <= segments; i++) {
            ...
            if (i > 0) {
                ...
                while (j--) {
                    (array as any)[j] = ((array[j] * radius) + middle).toFixed(2);
                }
        ...
        this.colorRing.hueMarker.css({
            fill: 'none',
            stroke: 'var(--ui-color-picker-marker-stroke)',
            strokeWidth: 'var(--ui-color-picker-marker-stroke-width)'
        });
    ...
    createGradient(type: string, props: Record<string, any>, colors: { offset: string; stopColor: string; stopOpacity: number }[]): Interface {
    ...
    onColorPicker = ({ open }: { open: boolean }) => {
        if (this.isOpen && !open) {
            this.close();
        }
    };
    ...
    setValue(value: Color | string | number, notify: boolean = true) {
        if (value && (value as Color).isColor) {
            this.value.copy(value as Color);
        } else {
    ...
    setHSL(h: number, s: number, l: number, notify: boolean = true) {
